...
import SimpleDialog from '../typescript/SimpleDialog.tsx';
interface Data{
    id:number,
    name:string,
    description:string
...
interface PaginationModel{
  ...
  const [name, setInputNombre] = useState<string>("");
  const [code, setInputCode] = useState<string|null>(null);
  const [open, setOpen] = useState<boolean>(false);
  const [selectRow, setSelectRow] = useState<Data|null>(null);
  ...
  useEffect(() => {
    ...
    fetch(`http://192.168.0.30:8080/snc-mf-api/v1/clients/${id}/procedures?pageSize=${paginationModel.pageSize}&page=${paginationModel.page}`)
      ...
      .then(data => (setPaginationModel((previus) => ({ ...previus, datos: data, loading: false, totalRows: data.totalRows }))));
  }, [id]);
  ...
  /*const handleSelectionChange= (newSelection:any) => {
    const selectId= newSelection[0]; 
    const selectedData=paginationModel.datos.find((row)=>row && row.id===selectId)
    setSelectRow(selectedData || null );
    setOpen(true);
  }*/
  ...
  const handleClickDelete = async (params:any) => {
    // setSelectRow(params.row); 
    try {
  ...
  return (
    <>
      ...
      <SimpleDialog  open={open} onClose={handleClose} onUpdate={handleActualizar} />
    </>
  )
}
