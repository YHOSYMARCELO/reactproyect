...
const MaterialForm :React.FC<Props> = ()=>{
  ...
  const [target1, setTarget1] = useState<any[]>([]);
  const [target2, setTarget2] = useState<any[]>([]);
      ...
      .then(data1 => {
        fetch(`${tagsUrl}?pageSize=${pagination.pageSize}&page=${pagination.page}`)
          ...
          .then(data2 => {
            ...
            setPagination(prev => ({ ...prev, totalRows: data1.totalRows, loading:false }));
          })
      })
